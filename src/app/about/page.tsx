...
// Fetch about page data server-side
async function getAboutPageData(): Promise<AboutPageData> {
  try {
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
    const response = await fetch(`${baseUrl}/api/page-settings/about`, {
    ...
    if (result.success) {
      ...
      throw new Error(result.message || 'Failed to fetch about page data');
    }
  } catch (error) {
